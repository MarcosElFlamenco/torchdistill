...
remote_distill:
	export WANDB_API_KEY=$(WANDB_API_KEY) ENV_MODEL_CONFIG=$(DISTILL_CONFIG) && echo $$ENV_MODEL_CONFIG && sky launch -c $(CLUSTER_NAME) --env WANDB_API_KEY --env ENV_MODEL_CONFIG skypilot/$(REMOTE_DISTILL_YAML) -i 10 --down 

remote_train_ce:
	export WANDB_API_KEY=$(WANDB_API_KEY) ENV_MODEL_CONFIG=$(CE_CONFIG) && echo $$ENV_MODEL_CONFIG && sky launch -c $(CLUSTER_NAME) --env WANDB_API_KEY --env ENV_MODEL_CONFIG skypilot/$(REMOTE_DISTILL_YAML) -i 10 --down 
